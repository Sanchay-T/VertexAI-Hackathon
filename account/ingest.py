...
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = 'ghc-030-1d73a9a52942.json'
...
def load_documents(data_dir: str) -> List[Document]:
    ...
    """
    dl = DirectoryLoader(data_dir, "**/*.pdf")
    documents = dl.load()
    return documents


def chunk_documents(documents: List[Document], chunk_size: int = 1400, chunk_overlap: int = 100) -> List[Document]:
    """Split documents into chunks

    Args:
        documents (List[Document]): A list of documents to split into chunks
        chunk_size (int, optional): The size of each chunk. Defaults to 1400.
        chunk_overlap (int, optional): The number of characters to overlap between chunks. Defaults to 100.

    Returns:
        List[Document]: A list of chunked documents.
    """
    pdf_text_splitter = RecursiveCharacterTextSplitter(chunk_size=chunk_size, chunk_overlap=chunk_overlap)
    split_documents = pdf_text_splitter.split_documents(documents)
...
def create_vector_store(documents, vector_store_path: str = "./vector_store") -> Chroma:
    ...
    vector_store = Chroma.from_documents(documents=documents, embedding=embedding_function, persist_directory=vector_store_path)
